...
def cut_and_align_face(image,box,Matrix):
    img = np.reshape(image, (192,192,3))
    img[:, :, 0] = image[2, :, :]
    img[:, :, 1] = image[1, :, :]
    img[:, :, 2] = image[0, :, :]
    dst = cv2.warpAffine(cv2.cvtColor(img,cv2.COLOR_RGB2GRAY), Matrix, (192,192))
    new_box = trans_points2d(box, Matrix)
    return dst[new_box[0]:new_box[2], new_box[1]:new_box[3]]
...
class Handler:
    ...
    def get(self, img, get_all=False):
        ...
        for i in range(bboxes.shape[0]):
            ...
            rimg = np.transpose(rimg, (2, 0, 1)) # 3*192*192, RGB
            input_blob[0] = rimg
